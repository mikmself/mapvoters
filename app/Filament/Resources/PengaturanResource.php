...
class PengaturanResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\TextInput::make('id_paslon')
                    ->required()
                    ->maxLength(255),
                Forms\Components\TextInput::make('target_suara')
                    ->required()
                    ->maxLength(255),
            ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('id_paslon')
                    ->primary()
                    ->searchable()
                    ->sortable(),
                Tables\Columns\TextColumn::make('target_suara')
                    ->searchable()
                    ->sortable(),
            ])
            ->filters([
